...
namespace LibDmd
{
	...
	public class RenderGraph : IRenderer
	{
					...
					if (Converter != null) {
						...
						if (sourceConverterColoredGray6 != null) {
							...
							// otherwise, convert to rgb24
								...
								Logger.Warn("Destination {0} doesn't support colored 6-bit frames from {1} converter, converting to Bitmap source.", dest.Name, sourceConverterColoredGray4.Name);
				...
				case FrameFormat.ColoredGray6:
					...
					switch (to)
					{
						...
						case FrameFormat.Rgb24:
							...
							Subscribe(sourceColoredGray6.GetColoredGray6Frames()
									.Select(frame => ColorUtil.ColorizeObsolete(source.Dimensions.Value, FrameUtil.Join(source.Dimensions.Value, frame.Planes), frame.Palette))
									.Select(frame => TransformRgb24(source.Dimensions.Value, frame.Data, destFixedSize)),
								destRgb24.RenderRgb24);
							break;
		...
		private DmdFrame ColorizeGray2(Dimensions dim, byte[] frame)
		{
			return ColorUtil.ColorizeObsolete(dim, frame, _gray2Palette ?? _gray2Colors);
		}

		private DmdFrame ColorizeGray4(Dimensions dim, byte[] frame)
		{
			return ColorUtil.ColorizeObsolete(dim, frame, _gray4Palette ?? _gray4Colors);
		}
		...
		private ColoredFrame TransformColoredGray2Obsolete(Dimensions dim, ColoredFrame frame, IFixedSizeDestination dest)
		{
			...
			return new ColoredFrame(dim, FrameUtil.Split(dest.FixedSize, 2, transformedFrame.Data), frame.Palette, frame.PaletteIndex);
		}

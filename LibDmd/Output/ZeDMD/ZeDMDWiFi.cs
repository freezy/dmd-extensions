...
namespace LibDmd.Output.ZeDMD
{
	...
	/// </summary>
	public class ZeDMDWiFi : ZeDMDWiFiBase, IGray2Destination, IGray4Destination, IColoredGray2Destination,
		IColoredGray4Destination, IRgb565Destination, IColoredGray6Destination, IMultiSizeDestination, IColorRotationDestination
	{
		...
		public static ZeDMDWiFi GetInstance(bool debug, int brightness, int rgbOrder, string port, string wifiAddress, int wifiPort)
		{
			if (_instance == null)
			{
				_instance = new ZeDMDWiFi { Debug = debug, Brightness = brightness, RgbOrder = rgbOrder, Port = port, WifiAddress = wifiAddress, WifiPort = wifiPort, WifiSsid = wifiSsid };
			}

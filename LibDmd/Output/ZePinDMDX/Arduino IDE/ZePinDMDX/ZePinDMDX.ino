...
#include <ESPPANE_HEIGHT-HUB75-MatrixPanel-I2S-DMA.h>
#include <SPIFFS.h>
...
void setup()
{
  ...
  mxconfig.mx_height = PANEL_HEIGHT;      // we have 64 pix high panels
  mxconfig.mx_width = PANEL_WIDTH;      // we have 64 pix wide panels
  mxconfig.chain_length = PANELS_NUMBER;  // we have 3 pannels chained
...
void loop()
{
  ...
  else if (c4 == 9) // mode 16 couleurs avec 1 palette 16 couleurs (16*3 bytes) suivis de 4 bytes par groupe de 8 points (séparés en plans de bits 4*512 bytes)
  {
    ...
    {
      Palette16[ti * 3] = img2[(15-ti)*3];
      Palette16[ti * 3 + 1] = img2[(15-ti)*3+1];
      Palette16[ti * 3 + 2] = img2[(15-ti)*3+2];
    }

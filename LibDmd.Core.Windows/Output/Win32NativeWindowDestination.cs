...
{
	internal sealed class Win32NativeWindowDestination : INativeWindowBackend, IGray2Destination, IGray4Destination, IGray8Destination, IRgb24Destination, IRgb565Destination, IFixedSizeDestination
	{
		...
		public Win32NativeWindowDestination(int width, int height)
			: this(width, height, 100, 100, width * Scale, height * Scale, false, VirtualDmdRenderStyle.Default)
		{
		}

		public Win32NativeWindowDestination(int width, int height, int windowLeft, int windowTop, int windowWidth, int windowHeight, bool stayOnTop)
			: this(width, height, windowLeft, windowTop, windowWidth, windowHeight, stayOnTop, VirtualDmdRenderStyle.Default)
		{
		}

		public Win32NativeWindowDestination(int width, int height, int windowLeft, int windowTop, int windowWidth, int windowHeight, bool stayOnTop, VirtualDmdRenderStyle renderStyle)
		{
		...
		public void Render(byte[] rgba)
		{
			...
			lock (_frameLock) {
				Buffer.BlockCopy(rgba, 0, _rgba, 0, Math.Min(rgba.Length, _rgba.Length));
			}
			RequestPaint();
		}
		...
		private sealed class NativeWindowClass
		{
			...
			private static readonly Dictionary<IntPtr, Win32NativeWindowDestination> Destinations = new Dictionary<IntPtr, Win32NativeWindowDestination>();
			...
			public static void SetDestination(IntPtr hwnd, Win32NativeWindowDestination destination)
			{
				lock (SyncRoot) {
			...
			private static bool TryGetDestination(IntPtr hwnd, out Win32NativeWindowDestination destination)
			{
				lock (SyncRoot) {

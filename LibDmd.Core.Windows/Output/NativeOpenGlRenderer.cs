	...
	internal sealed class NativeOpenGlRenderer : IDisposable
	{
		...
		private void Initialize()
		{
			...
			try {
				...
				Logger.Info("[DMD] Native dmdext OpenGL renderer initialized.");
			} catch (Exception exception) {
				_dmdRendererAvailable = false;
				Logger.Warn(exception, "[DMD] Native dmdext OpenGL renderer unavailable; using immediate dot renderer.");
			}
		}
		...
		private void RenderDmdExtPipeline(byte[] rgba, int clientWidth, int clientHeight, float offsetX, float offsetY, float renderWidth, float renderHeight)
		{
			...
			DrawDisplayQuad(offsetX, offsetY, renderWidth, renderHeight);
		...
		private static void DrawDisplayQuad(float x, float y, float width, float height)
		{
			DrawDmdQuad(x, y, width, height);
		}
